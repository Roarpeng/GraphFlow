...
## 7. v0.5.0 全语言索引精度与扩展生态适配完善 (2026-06-03)

本次更新在不改变原有图谱压缩与语义进化核心目标的基础上，大幅度提升了多语言跨平台的索引能力，并深度加固了开发插件与协议栈的集成体验。

### 7.1 全语言相对路径与模块映射解析
- **相对路径精准回溯**：原生接入 `node:path` POSIX 标准，彻底解决了 TypeScript、JS、C、C++ 项目中使用 `../` 或 `./` 的相对导入导致的图谱连线断层问题，所有相对引用均可精准收敛到基于项目根目录的绝对节点。
- **Python 包管理智能穿透**：重写了 Python AST 的 `import_from_statement` 规则，将 Python 独特的点号层级表示法（绝对如 `backend.src.module`，相对如 `from .utils`）映射为标准目录路径体系。

### 7.2 强缓存淘汰机制 (Cache Invalidation)
- 引入 `index-state.json` 结构的 Version 控制（当前版本 `version: 2`）。一旦系统检测到解析引擎的规则升级，自动丢弃旧版结构缓存，对所有文件无缝触发全量扫描，大幅度降低用户的维护心智负担。
