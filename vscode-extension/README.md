...
- Extension / runtime：**1.22.1**
- 市场身份：`roarpeng.graphflow`（displayName **GraphFlow Context & Memory**）
- 对应 VSIX：`../artifacts/graphflow-1.22.1.vsix`（本地打包）或 [GitHub Releases](https://github.com/Roarpeng/GraphFlow/releases)
...
```bash
code --install-extension graphflow-1.22.1.vsix
# Cursor CLI（若已安装）：
cursor --install-extension graphflow-1.22.1.vsix
```
...
1. 从 Releases 或本地 `artifacts/` 取得 `graphflow-1.22.1.vsix`
2. 按上文「安装 VSIX」步骤安装
...
- 或终端：`npx @roarpeng/graphflow@1.22.1 install`
