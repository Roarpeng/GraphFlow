...
- Extension / runtime：**1.7.13**
- 对应 VSIX：`../artifacts/graphflow-tool-1.7.13.vsix`（本地打包）或 [GitHub Releases](https://github.com/Roarpeng/GraphFlow/releases)
...
```bash
code --install-extension graphflow-tool-1.7.13.vsix
# Cursor CLI（若已安装）：
cursor --install-extension graphflow-tool-1.7.13.vsix
```
...
1. 从 Releases 或本地 `artifacts/` 取得 `graphflow-tool-1.7.13.vsix`
2. 按上文「安装 VSIX」步骤安装
...
- 或终端：`npx @roarpeng/graphflow@1.7.13 install`
...
- 升级到 **1.7.13+**，然后 Settings → **安装 / 更新 MCP**，Reload Window
