...
## Open retrieval dataset (download)

Third parties can download the **code-domain retrieval golden set** without
running TypeScript first:

| File | Format | Contents |
| --- | --- | --- |
| [`datasets/retrieval-golden-v1.json`](datasets/retrieval-golden-v1.json) | JSON (full document) | Metadata + 132 queries + 12 negatives |
| [`datasets/retrieval-golden-v1.jsonl`](datasets/retrieval-golden-v1.jsonl) | JSONL (one record/line) | Same entries, streamable |

- **License:** Apache-2.0 ([`LICENSE`](../LICENSE))
- **Source of truth:** `benchmarks/retrieval-golden-data.ts` (TypeScript). The
  JSON/JSONL are **generated** — after editing the TS data, run
  `npm run dataset:retrieval` so they cannot drift. Do not hand-edit the JSON.
- **Schema:** `schemaVersion`, `datasetId` (`retrieval-golden-v1`),
  `graphflowVersion` / `commit` notes, `domainCounts`, `evaluation` (how
  Hit@5 / MRR / NDCG@5 map to ranked anchors), then `queries` /
  `negativeSamples`.
- **Evaluate:** clone this repo at a pinned commit, `npm ci`, then
  `npm run bench:retrieval` (indexes this repo's `src/`, writes
  `.cache/retrieval-eval-results.json`).

Raw GitHub download (replace `<ref>` with a tag or commit):

```text
https://raw.githubusercontent.com/Roarpeng/GraphFlow/<ref>/benchmarks/datasets/retrieval-golden-v1.json
https://raw.githubusercontent.com/Roarpeng/GraphFlow/<ref>/benchmarks/datasets/retrieval-golden-v1.jsonl
```
...
with the regression suite `tests/retrieval-golden.test.ts` and published as
[`datasets/retrieval-golden-v1.json`](datasets/retrieval-golden-v1.json)) plus
12 negative (decoy-bleed) samples, on an in-memory graph built from this
repo's `src/`.
...
- Open dataset: `datasets/retrieval-golden-v1.json` (+ `.jsonl`) — downloadable
  golden queries; regenerate with `npm run dataset:retrieval`.
