...
export function validateConfigDetailed(path = "graphflow.config.json"): ConfigValidationResult {
  ...
  if (!parsed.learningPolicy) {
    ...
    if (parsed.learningPolicy.canaryRatio < 0 || parsed.learningPolicy.canaryRatio > 100) {
      issues.push({ severity: "error", field: "learningPolicy.canaryRatio", message: "canaryRatio must be 0-100" });
    }
  }
