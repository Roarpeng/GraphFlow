...
export function formatModelConfigGuide(workspaceRoot?: string): string {
  ...
  return [
    ...
    "",
    "## 3. 知识图谱语义提取（可选）",
    "- Settings → **Graph Semantic Enrichment**：可填 `deepseek-v4-flash` 等模型名",
    "- Provider / Model 均可留空，继承 Economy 层（适合 DeepSeek 单 provider 场景）",
    "- 也可使用本地 OpenBMB MiniCPM（见下方 OpenBMB 区块）",
    "",
    "## 4. 知识图谱何时更新？",
